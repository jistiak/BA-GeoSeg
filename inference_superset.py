...
def get_args():
    ...
    arg("-i", "--image_path", type=Path, required=True, help="Path to image folder or a single image file")
    arg("-c", "--config_path", type=Path, required=True, help="Path to config")
    arg("-o", "--output_path", type=Path, required=True, help="Path to save results")
    arg("-b", "--batch-size", type=int, default=1, help="Batch size")
    arg(
        "-d",
        "--dataset",
        default="none",
    ...
    arg(
        ...
        default=None,
        help=(
            "Optional JSON mapping image names to available_bands and band_to_index. "
...
# Dataset
# -----------------------------
class InferenceSupersetSingleDataset:
    """Wraps a single image for batched inference."""
    ...
    def __init__(
        ...
        self.img_tensor = pack_image_to_superset(
            image_hwc=image_hwc,
...
def main():
    ...
    for img_path in tqdm(img_paths, desc="Inferencing"):
        ...
        img_hwc = read_image_hwc(img_path)

        wrapper = InferenceSupersetSingleDataset(
            image_hwc=img_hwc,
            available_bands=available_bands,
            band_to_index=band_to_index,
            superset_bands=superset_bands,
        ...
        img_batch = wrapper.get_batch().cuda()

        with torch.no_grad():
            logits = model(img_batch)

            # Some model variants (e.g. those with deep supervision) return a
            # tuple/list of (main_output, aux_output, ...). Only the first
            # element is the primary segmentation logits.
        ...
        if args.save_raw or not args.save_color:
            ...
            tiff.imwrite(raw_out, pred)

        # Save color preview
        if args.save_color:
            color_mask = get_color_mask(pred, args.dataset)
